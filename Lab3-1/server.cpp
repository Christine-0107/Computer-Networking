...
int receiveMesg()
{
	...
	while(true)
	{
		...
		if (recvLength > 0) {
			...
			//校验和正确且序列号正确
			recv_C.fillData();
			if (recv_C.isCk(&recvPheader) && recvMesg.seq == curseq) {
				//消息为SYN
				if (recv_C.isSYN()) {
					sendSYNACK(recvMesg.seq);
				}
				//消息为FIN
			...
			//校验和错误或序列号错误
				...
				if (recv_C.isSYN()) {
					sendSYNACK((curseq + 1) % 2);
				}
				else if(recv_C.isFIN()){
					sendFINACK((curseq + 1) % 2);
				}
				else {
					sendACK((curseq + 1) % 2);
				}
